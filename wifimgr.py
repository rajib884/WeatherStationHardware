...
def handle_configure(client, request):
    ...
    print(f"SSID:{match.group(1)}")
    print(f"Pass:{match.group(2)}")
    try:
        ssid = match.group(1).decode(
            "utf-8").replace('+', ' ').replace("%3F", "?").replace("%21", "!")
        password = match.group(2).decode(
            "utf-8").replace('+', ' ').replace("%3F", "?").replace("%21", "!")
    except Exception:
        ssid = match.group(1).replace('+', ' ').replace("%3F", "?").replace("%21", "!")
        password = match.group(2).replace('+', ' ').replace("%3F", "?").replace("%21", "!")
